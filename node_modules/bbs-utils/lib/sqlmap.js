...
var sqlMap = {
    ...
    userInfo: function () { //用户信息
        let sql = "SELECT user.id AS id,username,nickname,headimg,level,socketid,isonline,escape,flappyBird ";
        sql += "FROM user LEFT JOIN score ";
        sql += "ON user.id=score.id";
        return sql;
    },
    hasUser: function () { // 用户名是否存在
    ...
    scoreRank: function (gameName) { // 排名
    ...
    /**
     * 获取表的状态，Auto_increment当前表的自增最大id
     ...
     * @param {String} conditionSymbol AND | OR
     */
    whereAO: function (sql, conditions, relationSymbol, conditionSymbol) {
        let cSymbol = conditionSymbol || "AND";
        if (conditions && conditions.length > 0) {
            ...
            for (let i = 0; i < conditions.length; i++) {
                ...
                sql += " " + cSymbol+ " " + condition + relationSymbol;
            }
            return sql;
        } else {
    ...
    whereAnd: function (sql, conditions, relationSymbol) {
        return this.whereAO(sql, conditions, relationSymbol, "AND");
    },
    /**
     * 添加 where or 条件
     * @param {String} sql
    ...
    whereOr: function (sql, conditions, relationSymbol) {
        return this.whereAO(sql, conditions, relationSymbol, "OR");
    },
    /**
     * 获取数量
     * @param {String} table 
